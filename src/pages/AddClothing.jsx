...
import SelectTagList from '../components/Clothing/SelectTagList';
...
const AddClothing = (props) => {
	...
	useEffect(() => {
		if (props.clothingId != null && props.clothingId != '')
			setClothingId(props.clothingId);
	}, []);
	...
	useEffect(() => {
		...
		const getClothingByIdCallback = (result) => {
		...
		if (clothingId)
			sendRequest(
				...
				getClothingByIdCallback
			);
	}, [clothingId, sendRequest]);
	...
	const submitHandler = (event) => {
		...
		if (clothingId)
			sendRequest(
				{
					url: 'clothing/xu7Di7YPp4hvrN250XWwqcy7YVLY/' + clothingId,
					method: 'PUT',
	...
	return (
		...
		<form
			...
			className='flex flex-col md:flex-row md:items-stretch space-y-8 md:space-y-0 md:space-x-2 justify-center items-center'
		>
			...
			<div className='w-full flex flex-col space-y-5'>
				...
				<div className='flex justify-end'>
					<button className='py-2 px-1 w-1/3 hover:bg-purple-400 hover:text-white rounded border-2 border-purple-400'>
						Add Clothings
					</button>
				</div>
			</div>
